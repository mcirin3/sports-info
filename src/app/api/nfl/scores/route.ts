...
export async function GET(req: NextRequest) {
  try {
    ...
    const url = `https://site.api.espn.com/apis/site/v2/sports/football/nfl/scoreboard?dates=${espnDate}&limit=300`;
...
function mapEspnEventToGame(ev: any): GameLite {
  ...
  const tv = broadcasts.flatMap((b: any) => (Array.isArray(b?.names) ? b.names : [])).filter(Boolean);
